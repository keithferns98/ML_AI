...
from multidoc_chat.utils.file_io import save_uploaded_files
from multidoc_chat.utils.document_ops import load_documents
...
class FaissManager:
    def __init__(self, index_dir: Path, model_loader: Optional[ModelLoader] = None):
        ...
        if self.meta_path.exists():
            try:
                self._meta = json.loads(self.meta_path.read_text(encodings = "utf-8")) or{"rows": {}}
            except Exception as Error:
                self._meta = {"rows":{}}
            
        self.model_loader = ModelLoader()
        self.embeds= self.model_loader.load_embeddings()
